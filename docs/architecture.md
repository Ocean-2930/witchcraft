...
- 적 생성용 기본 정보는 `data/definitions/enemies.json`에 코드별 `UnitBase` 필드로 둔다. 현재 코드는 `basic_monster`이며 기존 이름·체력 100·공격력 0과 나머지 기본 수치를 유지한다. `units/enemy_definitions.py`는 UTF-8 파일을 실행 위치와 무관하게 한 번 읽고 필수 필드, 숫자 타입·유한값, 체력·마나 최솟값, 속도 범위, 중복 키와 알 수 없는 필드를 검사한다. 이름·최대 체력·공격력은 필수이고 나머지는 생략 시 `UnitBase` 기본값을 사용한다. 정의와 능력치 매핑은 읽기 전용이다.
- `Enemy.from_code(code, tile_x=..., tile_y=...)`는 정의에서 독립적인 적을 생성한다. `DungeonScene.create_monster()`는 기본적으로 `basic_monster`를 사용하며 선택적 `enemy_code`로 다른 정의도 선택할 수 있다. JSON에 종류를 추가해도 무작위 소환 목록에 자동으로 들어가지는 않는다. AI 동작은 기존 `Enemy` 및 던전 행동 로직을 유지한다. 파일 수정은 재시작 후 새로 생성되는 적부터 적용한다.
