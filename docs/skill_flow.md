...
- `DungeonScene.use_hotbar_skill(...)`은 대상 타일 순서대로 살아 있는 몬스터를 수집하고 `DungeonInventory.use_skill(...)`에 시전을 한 번 요청한다. 대상이 없어도 유효한 방향과 사용 조건을 만족하면 허공 시전으로 처리한다.
- `SkillBase.use_targets(...)`는 시전 비용을 한 번만 지불한 뒤 각 대상에 활성 효과를 적용한다. 기존 단일 대상 `use(...)`는 이 경로에 위임한다.
- `DungeonInventory`는 장비와 패시브가 반영된 전투 능력치 복사본으로 스킬을 실행하고, 실행 후 `hp`, `mp`, `buffs`를 원본 플레이어에 반영한다.
- 성공한 시전은 공격 행동 비용만큼 전투 시간을 진행한다. `skillbase` 아이템은 대상 유무와 관계없이 성공 시 한 개를 소비한다.
- 피해로 죽은 몬스터는 다음 몬스터 행동을 처리하기 전에 scene, renderer와 전투 타이머에서 제거한다.

## 스킬별 피해 계산

- `SkillCastContext`는 스킬 정의, 합산 레벨, 시전자와 현재 대상을 효과 및 피해 계산에 전달한다. 아이템의 `skillbase`는 레벨 1로 실행한다.
- `SkillBase`는 명중률, 치명타 확률, 방어, 초과 관통, 치명타 피해, 피해 증가와 최종 피해 확정 단계별 callable을 선택적으로 제공한다.
- callable의 기본값은 `None`이다. `DamageBlock`에 callable이 없으면 `Unit`의 기존 계산 메서드를 사용하고, 있으면 해당 단계의 기본 계산을 완전히 대체한다.
- 실제 적용과 `peek(...)`은 동일한 callable 경로를 사용한다. 최종 피해 callable이 없을 때만 기존 `max(1, floor(damage))` 규칙을 적용한다.
