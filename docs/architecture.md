...
- 정의 객체는 세이브에 복사하지 않는다. 기존 아이템 코드와 인스턴스 수량·장비 스탯 행 저장 방식을 유지한다. 복원 시 설치된 최신 정의를 사용하므로 설명·회복량 변경은 기존 보유 아이템에도 적용된다. 최대 스택을 저장 수량보다 줄이는 등 호환되지 않는 변경은 별도 세이브 변환이 필요하며 기존 검증이 이를 거부한다. 적 데이터와 UI 흐름, 상속 구조는 변경하지 않는다.
...
- 파일은 실행 디렉터리와 무관하게 저장소의 `data/saves/continue.json`에 UTF-8로 기록한다. `save_version=3`으로 형식을 구분하고 버전 1·2를 변환하여 읽으며 그 외 버전은 명확히 거부한다. 형식 변경 시 변환기를 추가해야 한다. 런타임 저장 파일은 Git에서 제외한다.
...
- 아이템 테이블은 스냅샷 안에서 고유한 ID를 부여한다. 인벤토리, 장비, 바닥 아이템은 ID를 보관하며 단축키는 동일 인스턴스를 참조한다. 종류는 `item_code`와 등록된 생성 함수로 복원하고, 장비의 합성 결과는 각 인스턴스의 `stat_rows`로 기록한다. 새 아이템·스킬 콘텐츠는 아이템은 `items/registry.py`, 스킬은 `serializers.py`의 코드 레지스트리에 등록한다.
...
- 적의 `drop_items`는 `(code, weight)` 항목의 불변 튜플이며 JSON에서는 `[[code, weight], ...]`로 표현한다. 문자열 코드는 지정 아이템, 정수 0은 드롭 없음, 정수 1은 미래 랜덤 장비 드롭용 예약 분기다. 현재 1이 선택되면 아이템을 생성하지 않는다. 가중치는 유한한 0 이상 숫자이며 0은 선택되지 않고, 빈 표나 합계 0도 드롭하지 않는다. 고블린은 `[["simple_sword", 1], [0, 0]]`, 슬라임은 `[["blue_potion", 1], [0, 0]]`로 각각 한 개를 확정 드롭한다.
- `utilities.dungeon.item_drops`는 층별 아이템 난수로 표 전체에서 한 항목만 선택한다. 드롭과 저장 복원은 `items/registry.py`의 생성 레지스트리를 공유한다. 사망 제거 시 `DungeonScene`은 결과를 사망 좌표의 `DungeonMap.ground_items`에 추가하며, 장비는 고유한 스탯 행을 갖는 `EquipmentInstance`로 생성한다. 기존 바닥 아이템은 유지하고 살아 있는 적 제거에는 드롭하지 않는다. 재화 보상과 별개로 처리하며 행운·장비 드롭률 보정은 가중치에 적용하지 않는다.
- 버전 3은 살아 있는 적의 드롭 표를 저장·검증·복원한다. 이미 바닥에 떨어진 아이템은 기존 아이템 ID 테이블과 장비 스탯 행 경로로 저장한다. 층별 아이템 난수 상태도 유지하므로 이어하기 후 추첨 흐름을 재현한다. 상속 구조와 스킬 실행 흐름은 변경하지 않는다.

- `GroundItemsRenderer`는 던전의 바닥 아이템 표시를 담당한다. scene은 그리기 직전 맵의 바닥 목록·현재 시야·바닥 renderer를 기준으로 타일별 renderer를 동기화하며 제거에는 `destroy()`를 사용한다. 바닥 renderer의 중심을 따라 이동하고 바닥과 유닛 사이 깊이로 정렬한다. 모델이나 저장 형식은 변경하지 않는다.
