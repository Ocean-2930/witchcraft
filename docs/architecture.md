...
- `utilities.dungeon.navigation`은 벽과 동적 점유 칸을 피해 시작점을 제외한 8방향 최단 경로를 반환한다. 양쪽 직교 칸이 모두 막힌 대각선 모서리는 통과하지 않는다. `Enemy`는 기본 보초 모드에서 적 전용 난수 흐름으로 도달 가능한 일반 바닥 목적지를 선택하고 한 행동에 한 칸씩 이동한다. 행동 직전 공용 시야에서 플레이어를 발견하면 전투 모드로 전환해 인접 칸까지 추적하고, 시야를 잃으면 다시 보초 모드로 돌아간다. 플레이어 이동 비용을 진행하는 동안 준비된 적은 등록 순서대로 모두 행동하고 각자의 이동 비용으로 다시 예약된다.
