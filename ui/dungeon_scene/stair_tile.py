...
class StairTileRenderer(Renderer):
    ...
    def __init__(self, scene, pos_x, pos_y, width, height, texture_key):
        ...
        floor_size = (int(width), int(height))
        ...
        if floor_size not in self.__class__.floor_images:
            self.__class__.floor_images[floor_size] = DUNGEON_TEXTURES.get_scaled(
                "floor",
                width,
                height,
            )
        ...
        self.floor_image = self.__class__.floor_images[floor_size]
        super().__init__(scene, pos_x, pos_y, width, height)

    def draw(self, screen):
        previous_clip = screen.get_clip()
        screen.set_clip(screen.get_rect())
        if self.floor_image is not None:
