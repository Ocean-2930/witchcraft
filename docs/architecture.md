...
|---|---|---|
| `core` | 실행 루프, 입력 수집, 화면·사운드 환경, 진행 세션의 활성화와 저장 시점 조정 | 모델 내부 계산 규칙 |
...
- `settings.REST_TICKS=100`은 `DungeonScene.rest_one_turn()`의 시간 진행량과 타임라인의 휴식 예고가 공유한다. G 입력은 keydown에서 한 번 처리하며 기존 `advance_monster_turns()`를 통해 적 행동과 턴 카운터를 진행한다.
...
- `DungeonMap`은 현재 층의 `ground_items`(좌표 → `ItemInstance` 목록)와 JSON 값으로 표현되는 `event_states`를 소유한다. `DungeonScene.ground_items`는 이 목록을 참조하며, `tile_interactions`(좌표 → `callback(scene)`)는 실행 중 scene이 소유한다. `T` 입력은 `interact_with_current_tile()`로 전달하며, 아이템 보관 성공 여부는 `DungeonInventory.add_item()`에 위임한다. 아이템이 있는 타일은 줍기만 처리하고 이벤트는 다음 입력에서 실행한다. 이벤트의 일회성 여부와 행동 시간 비용은 callback이 관리하며, 현재 줍기 자체는 전투 시간을 진행하지 않는다.
...
- 게임 진입 중 선택한 고정 seed는 `Game`이 실행 범위의 공통 상태로 보관한다. 게임을 시작하면 선택했거나 새로 만든 게임 seed를 `DungeonInventory.game_seed`에도 기록한다. 진입 화면의 고정 seed 선택은 취소, 상태 마커 클릭 또는 게임 종료 시 사라지지만, 시작한 진행의 `game_seed`는 세이브에 기록한다.
...
- `EquipmentInstance.synthesis_cost()`는 결과 행의 분류와 레벨로 조화석 비용을 계산한다. `DungeonInventory.synthesize_equipment()`는 소유·규칙·잔액 검증을 모두 마친 뒤 재료 제거, 메인 행 적용, 조화석 차감과 재료의 단축키 참조 정리를 수행한다. 메인은 인벤토리 또는 장착 슬롯에서 동일 객체 참조로 소유 여부를 검증하며, 재료는 인벤토리 장비만 허용한다. 장착 중 합성은 슬롯 참조를 유지하고 메인의 행만 갱신하므로 기존 저장·복원 형식을 사용한다. 미리보기는 원본을 바꾸지 않으며 실행 시 다시 계산한다.
...
- 적 생성용 기본 정보는 `data/definitions/enemies.json`에 코드별 `UnitBase` 필드로 둔다. 현재 종류는 `basic_monster`(슬라임, 체력 100·공격력 5), `cave_bat`(동굴 박쥐, 체력 60·공격력 8·이동 속도 1), `goblin`(고블린, 체력 120·공격력 12·방어력 3)이다. `units/enemy_definitions.py`는 UTF-8 파일을 실행 위치와 무관하게 한 번 읽고 필수 필드, 숫자 타입·유한값, 체력·마나 최솟값, 속도 범위, 중복 키와 알 수 없는 필드를 검사한다. 이름·최대 체력·공격력은 필수이고 나머지는 생략 시 `UnitBase` 기본값을 사용한다. 정의와 능력치 매핑은 읽기 전용이다.
- `Enemy.from_code(code, tile_x=..., tile_y=...)`는 정의에서 독립적인 적을 생성한다. `DungeonScene.create_monster()`는 코드 생략 시 층별 적 전용 난수로 `SPAWN_ENEMY_CODES`의 3종 중 하나를 동일 확률로 선택한다. 초기·주기 스폰 모두 이 경로를 사용하며, 명시적 `enemy_code`는 종류 추첨 없이 해당 정의로 생성한다. JSON에 종류를 추가해도 명시적인 소환 목록에 자동으로 들어가지는 않는다. 종류 선택도 적 난수 상태를 소비하므로 동일 시드·동일 행동 순서에서 재현되며, 기존 저장 상태 복원에는 추첨하지 않는다. AI 동작은 기존 `Enemy` 및 던전 행동 로직을 유지한다. 파일 수정은 재시작 후 새로 생성되는 적부터 적용한다.
- 적 정의 코드는 생성 템플릿 조회에 사용한다. 적의 기본 지급량 `drop_gold`·`drop_harmony_stones`는 0 이상의 정수이며 생략 시 0이다. 기본값은 슬라임 10·1, 동굴 박쥐 15·1, 고블린 25·2이다. 기존 `gold_drop_amount`는 지급량이 아닌 획득 보너스 능력치다. 사망한 적을 제거할 때 합산 플레이어 능력치의 골드 획득 공식(행운·골드 보너스)을 적용하고 조화석은 정의 수량 그대로 지급한다. 목록에서 제거한 적은 중복 지급하지 않는다. 지급량은 적 인스턴스에 저장하여 정의 변경 후 이어하기에도 유지한다. 저장 형식은 버전 2이며 버전 1의 기존 적은 지급량 0으로 복원한다. 새로 생성되는 적에는 최신 정의가 적용된다.

- 아이템 공통 정보는 `data/definitions/items.json`의 안정적인 아이템 코드를 키로 관리한다. `items/definitions.py`가 실행 위치와 무관한 절대 경로에서 UTF-8로 한 번 읽고 필수 필드·타입·범위·중복 키를 검증해 불변 `ItemDefinition`으로 제공한다. 실제 콘텐츠 클래스는 정의를 공유하고 공통 이름·설명·플레이버 표시는 `Item`이 처리한다. 물약 사용 같은 동작은 Python 클래스에 남긴다. JSON 변경은 게임 재시작 시 적용된다.
- 정의 필드는 `name`, `description`, `flavor_text`, `max_stack`과 선택 필드 `mp_recovery`, `equip_type`이다. 설명의 `{mp_recovery}`는 실제 회복량으로 치환한다. 장비 최대 스택은 1이며 장비 종류는 weapon/sub_weapon/armor/accessory 중 하나다. 새 콘텐츠는 JSON뿐 아니라 필요한 구현 클래스와 저장 레지스트리에도 등록한다.
- 정의 객체는 세이브에 복사하지 않는다. 기존 아이템 코드와 인스턴스 수량·장비 스탯 행 저장 방식 및 `save_version=1`을 유지한다. 복원 시 설치된 최신 정의를 사용하므로 설명·회복량 변경은 기존 보유 아이템에도 적용된다. 최대 스택을 저장 수량보다 줄이는 등 호환되지 않는 변경은 별도 세이브 변환이 필요하며 기존 검증이 이를 거부한다. 적 데이터와 UI 흐름, 상속 구조는 변경하지 않는다.

- `utilities.save.GameSession`은 인벤토리, 현재 층 번호, 생성된 층별 `DungeonMap`을 소유한다. `Game`은 활성 세션과 던전 scene을 연결하고, `SaveManager`에 저장을 요청한다. 직렬화 모듈은 scene·UI를 참조하지 않는다.
- 파일은 실행 디렉터리와 무관하게 저장소의 `data/saves/continue.json`에 UTF-8로 기록한다. `save_version=1`로 형식을 구분하고 현재 다른 버전은 명확히 거부한다. 형식 변경 시 변환기를 추가해야 한다. 런타임 저장 파일은 Git에서 제외한다.
- 새 던전 진입 직후 저장한다. 이후 0.5초마다 이동 애니메이션이 끝난 상태에서 변경 여부를 확인해 저장한다. 인벤토리·합성·설정 overlay의 변경도 포함한다. 메인으로 복귀하거나 창 닫기/게임 종료를 요청하면 진행 중인 이동 한 번과 그에 따른 몬스터 처리를 완료한 뒤 저장한다. 강제 프로세스 종료는 마지막 자동 저장 이후의 변경을 보장하지 않는다.
- 아이템 테이블은 스냅샷 안에서 고유한 ID를 부여한다. 인벤토리, 장비, 바닥 아이템은 ID를 보관하며 단축키는 동일 인스턴스를 참조한다. 종류는 `item_code`와 등록된 생성 함수로 복원하고, 장비의 합성 결과는 각 인스턴스의 `stat_rows`로 기록한다. 새 아이템·스킬 콘텐츠는 `serializers.py`의 코드 레지스트리에 등록한다.
- 플레이어는 한 번만 복원하고 모든 맵·인벤토리가 공유한다. 몬스터의 능력치·AI 상태, 타이머의 유닛 참조·등록 순서·남은 틱, 턴 카운터를 복원한다. 현재 HP/MP는 장비 보정 전 기본 최대치로 다시 제한하지 않는다.
- 맵은 seed로 재생성하지 않고 타일·방·연결·계단·이벤트·발견 기록을 복원한다. 층별 용도별 난수는 `RandomGenerator.from_state()`로 현재 상태를 그대로 복원한다. 초기 몬스터 생성과 테스트 시나리오는 이어하기에서 반복하지 않는다.
- 버프와 이벤트 진행 상태는 JSON 기본 자료형으로 저장한다. 현재 구체적인 버프 클래스와 이벤트 실행 레지스트리는 없으므로, 향후 객체형 버프나 타일 callback 콘텐츠를 도입할 때 코드 기반 직렬화·복원 연결을 추가해야 한다. 함수·이미지·renderer·조준·합성창의 임시 선택은 저장하지 않는다. 이벤트 callback 자체는 저장 대상이 아니다.
- 저장 후보를 복원 경로로 검증한 뒤 임시 파일에 쓰고 flush/fsync 후 원자적으로 교체한다. 직전의 정상 저장은 `continue.bak`에 보관한다. 이어하기는 본 JSON만 사용하며 손상 시 자동으로 백업을 채택하지 않는다. 오류를 알리고 원본을 유지한다. 새 게임 확인 시 본 JSON과 백업을 삭제한다.
- 저장 실패는 안내 overlay로 표시한다. 실패한 종료·메인 복귀는 중단하여 현재 진행을 메모리에 유지하며, 읽기 실패도 기존 파일을 삭제하지 않는다.
