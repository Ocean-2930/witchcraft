...
- `DungeonScene.combat_timer`는 플레이어와 생성된 몬스터의 다음 행동까지 남은 정수 틱을 관리한다. 속도 단계 `N`의 기본 행동 비용은 `int(2 ** (-N / 3) * 100)`이며, 타이머는 가장 가까운 행동 시점만큼 모든 값을 줄여 동시에 0이 된 유닛들을 반환한다. 실제 행동을 결정한 뒤에는 공격 또는 이동에 맞는 비용으로 해당 유닛을 다시 예약한다. 현재 입력과 이동에는 아직 이 진행을 자동 연결하지 않는다.
