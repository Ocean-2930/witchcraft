...
장비 합성은 전투 스킬 레벨 합산과 별개다. 같은 코드·같은 레벨이면 +1하고, 같은 코드·다른 레벨이면 메인 장비 레벨을 유지한다. 재료에만 있는 스킬은 그대로 추가한다. 결과는 승급·공통·메인 전용·재료 전용 순서로 정렬하며 실제 스킬 행이 7개 이상이면 선택할 수 없다. 미리보기는 원본 스킬 인스턴스를 변경하지 않는다. 합성을 실행하면 검증 후 결과 행을 메인 장비에 적용하고 재료 및 필요한 조화석을 소비한다.
