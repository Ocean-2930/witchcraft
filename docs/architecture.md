...
- 공용 `ItemWindow`는 `ItemInstance` getter만 받아 인벤토리 슬롯과 단축키 슬롯에서 재사용하며, 구체 아이템 이름·설명·상세 행·플레이버 문구는 각 `Item` 클래스가 소유한다. `SkilledEquip`은 보유 스킬과 빈 스킬 슬롯을 상세 행으로 변환한다.
