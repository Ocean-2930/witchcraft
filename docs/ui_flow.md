...
- 던전에서는 아무 지형도 없는 배경을 순수 검정으로 채우고 플레이어의 현재 시야만 정상 밝기로 표시한다. 한 번 본 지형은 검은 안개로 남고 미발견 지형과 현재 시야 밖 몬스터는 표시하지 않는다. 방 밖 시야는 정사각형 반경 4칸이지만 중간 벽 뒤쪽은 가려지며, 보이는 통로 바닥에 직접 닿은 주변 벽은 함께 밝힌다. 방 안에서는 방 바닥과 경계벽 전체를 밝힌다.
