...
- 던전 HUD의 프로필 아래 전투 타임라인은 합산된 공격·이동 속도를 행동 비용으로 변환해 다음 공격과 다음 이동을 각각 한 번씩만 표시하며, 이후 적 시점을 포함해도 전체 휘장은 최대 8개로 제한한다. 휘장 안에는 왼쪽 위 프로필과 같은 영웅 이미지 및 검·신발 행동 표식을 함께 사용하고, 휘장 위에는 현재부터 해당 행동까지 필요한 틱을 표시한다. 절대 시점은 사용하지 않으며 0~99의 `TurnCounter`를 기준으로 다음 100틱 행동 구간까지 남은 값과 이후 구간을 세로 구분선 아래에 표시한다. 붉은 휘장 자산은 이후 적 행동 시점 표시에 사용한다.
