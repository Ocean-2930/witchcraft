...
영웅 탭에서 마우스 휠은 포인터가 올라간 티어 열의 카드를 위아래로 스크롤한다. 티어명과 해당 티어의 Skill Point는 카드 스크롤 상자와 분리된 위쪽 고정 헤더 상자에 표시된다. 카드 상자의 높이를 늘리지 않고 아래 여백을 넓혀 가로 스크롤 입력 영역을 확보하며, 가로 스크롤바는 패널 아래쪽에 붙여 표시한다. 티어 열 바깥의 이 영역에서는 전체 티어 목록을 좌우로 스크롤한다. 공용 `SkillCard`는 아이콘, 스킬명과 왼쪽 정렬된 현재 레벨만 표시하고 투자 동작을 소유하지 않는다. 영웅 스킬 노드 목록은 카드와 별도의 `SkillInvestButton`을 오른쪽에 겹쳐 배치하며 포인트가 없거나 최대 레벨이면 버튼을 비활성화한다.

스킬 카드의 아이콘에 마우스를 올리면 공용 `SkillInfoWindow`가 포인터 옆에 나타난다. 검은 정보창 첫 줄에는 `스킬명 Lv.N`을 표시하고 구분선 아래에는 `SkillBase.description`을 너비에 맞춰 줄바꿈한다. 화면 공간을 넘는 내용은 말줄임표로 자른다. `SkillCard`와 `SkillInfoWindow`는 생성 시 `SkillInstance`를 직접 받아 다른 화면에서도 같은 표시 규칙으로 재사용할 수 있다.
