...
- 적 정의 코드는 현재 생성 템플릿 조회에만 사용한다. 적은 기존과 같이 모든 능력치·이름·현재 체력·위치·AI 상태를 세이브에 기록하고, 복원할 때 생성 정의를 다시 적용하지 않는다. 새 런타임 필드나 저장 필드가 없으므로 기존 `to_data`/`from_data`와 `save_version=1`을 유지한다. 종류별 행동이나 코드 참조가 필요한 상태를 추후 도입하면 적 코드 저장과 버전 호환 처리를 함께 추가해야 한다. 이번 변경은 클래스 상속 구조와 UI·스킬 호출 흐름에 영향을 주지 않는다.
